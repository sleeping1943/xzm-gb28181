...
#define BEGIN_REGISTER_EVENT_HANDLER                               \
    ...
    {
#define REGISTER_EVENT_HANDLER(event, handler) {event, std::make_shared<handler>()}
...
#define REGISTER_MSG_RESPONSE(msg, handler, ptr)                                                       \
    {msg, std::bind(handler, ptr, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, \
                    std::placeholders::_4)},
#define END_REGISTER_MSG_RESPONSE() \
    }                               \
    ;
#define SIP_STRDUP(field) \
    if (auth->field) (field) = osip_strdup_without_quote(auth->field)
...
#define HV_REGISTER_ASYNC_HANDLER(router, type, path, func, obj_ptr) \
    router.type(path, [obj_ptr](const HttpContextPtr &context) { return obj_ptr->func(context); });
...
struct Client
{
    Client(const std::string &_ip, unsigned short _port, const std::string &_device)
        : ip(_ip), port(_port), device(_device), is_talking(false)
    {
    }

    Client(const std::string &_ip, unsigned short _port, const std::string &_device, bool _is_reg,
           unsigned short _rtp_port, XClientType _client_type)
        : ip(_ip),
          port(_port),
          device(_device),
          is_reg(_is_reg),
