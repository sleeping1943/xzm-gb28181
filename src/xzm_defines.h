...
/* 国标消息处理函数定义 */
#define BEGIN_REGISTER_MSG_RESPONSE(resps) resps = {
#define REGISTER_MSG_RESPONSE(msg, handler, ptr)                               \
  {msg, std::bind(handler, ptr, std::placeholders::_1, std::placeholders::_2,  \
...
/* libhv的json解析 */
#define HV_JSON_GET_INT(json, to, key)                                         \
  HV_JSON_GET_VALUE(json, to, key, number_integer)
#define HV_JSON_GET_STRING(json, to, key)                                      \
  HV_JSON_GET_VALUE(json, to, key, string)
#define HV_JSON_GET_BOOL(json, to, key)                                        \
  HV_JSON_GET_VALUE(json, to, key, boolean)
#define HV_JSON_GET_VALUE(json, to, key, type)                                 \
  auto iter_##to = json.find(key);                                             \
...
/* xml解析 */
#define XML_GET_STRING_DEFAULT(node, name, value, temp_node, temp_text,        \
                               default_string)                                 \
  {                                                                            \
...
enum HttpCode {
  ...
  kHttpMultipleChoices =
      300, // 多种选择。请求的资源可包括多个位置，相应可返回一个资源特征与地址的列表用于用户终端（例如：浏览器）选择
  kHttpMovedPermanently =
  ...
  kHttpProxyAuthenticationRequired =
...
public:
  ...
  std::string
      indistinct_query; // 可选 录像模糊查询属性,缺省为0 0-不进行模糊查询
  unsigned short stream_number =
      0; // 可选 码流编号 0-主码流 1-子码流 2-子码流 3-以此类推
...
public:
  ...
  virtual std::string
  ExtroXmlQueryParamfo(const XmlQueryParamPtr &msg_in) override;
};
...
struct ServerInfo {
  ...
  std::string str() {
    ...
    ss << std::endl
       ...
       << "\tip:\t" << ip << std::endl
       << "\tport:\t" << port << std::endl
       << "\tsip_id:\t" << sip_id << std::endl
       << "\trealm:\t" << realm << std::endl
...
struct ClientInfo {
  ...
  int parental = 0;
  std::string
      parent_id; // parent_id和client中的device_id相同时，该设备具有语音输出功能
  int register_way = 1;
  int safety_way = 0;
  int secrecy = 0;
  int status = 1;
...
struct Client {
  Client(const std::string &_ip, unsigned short _port,
      ...
      : ip(_ip), port(_port), device(_device), is_talking(false) {}

  Client(const std::string &_ip, unsigned short _port,
         const std::string &_device, bool _is_reg, unsigned short _rtp_port,
         XClientType _client_type)
      : ip(_ip), port(_port), device(_device), is_reg(_is_reg),
        rtp_port(_rtp_port), client_type(_client_type), is_talking(false) {}
  ...
  std::unordered_map<std::string, ClientInfoPtr>
      client_infos_; // 每个设备有多个信息，如摄像头有视频和音频2个设备信息
