...
public:
    ...
    bool UpdateClientInfo(const std::string &device_id, std::unordered_map<std::string, ClientInfoPtr> client_infos);
     ...
     */
    void UpdateClientAccessTime(const std::string &device_id);

public:
    static HandlerPtr kDefaultHandler;
    static PtzHandlerPtr kPtzHandler;

    std::map<std::string, bool> living_states_;  // <id, state>
...
private:
    ...
    std::unordered_map<std::string, std::vector<RecordInfoPtr>> record_infos_;  // 缓存的历史录像记录 <device_id,
