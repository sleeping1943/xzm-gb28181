...
public:
  ~Server();
  inline void Test() { LOG(INFO) << "server Test"; }
  ...
  bool
  UpdateClientInfo(const std::string &device_id,
  ...
  void AddRecordInfo(const std::string &parent_device_id,
                     std::vector<RecordInfoPtr> records);
...
public:
  ...
  // static std::unordered_map<std::string, StreamInfo> stream_infos_;   //
  // <stream_id, StreamInfo>,stream_id以"talk_"开头

private:
  std::atomic_bool is_quit_;
  struct eXosip_t *sip_context_;
  std::thread thread_;
  std::unordered_map<eXosip_event_type, HandlerPtr>
      event_map_; // 注册的事件处理函数体
  std::unordered_map<std::string, ClientPtr>
      clients_; // 已注册的客户端 <device_id, std::shared_ptr<Client>>
  std::unordered_map<std::string, std::vector<RecordInfoPtr>>
      record_infos_; // 缓存的历史录像记录 <device_id,
                     // std::vector<RecordInfoPtr>>
  ...
  std::unordered_map<std::string, std::pair<int, int>>
      publish_streams_; // 正在推流直播的rtsp, <ssrc, <cid, did>>
  std::mutex talk_mutex_;
  std::queue<unsigned short> talk_ports_;

  B_Lock valid_stream_mutex_;
  std::mutex stream_mtx_;
  std::unordered_map<std::string, StreamInfoPtr>
      stream_infos_; // <stream_id, stream_info>
};
