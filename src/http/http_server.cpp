...
bool XHttpServer::Init(const std::string &conf_path) {
  ...
  JSON_VALUE_OPTION_INT(http_config, "snap_cache_time", s_info_.snap_cache_time,
                        3600);
  ...
  /* 同步请求 */
  HV_REGISTER_SYNC_HANDLER(router, GET, "/query_device", XHttpServer,
                           query_device_list, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/start_rtsp_publish", XHttpServer,
                           start_rtsp_publish, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/stop_rtsp_publish", XHttpServer,
                           stop_rtsp_publish, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/start_invite_talk", XHttpServer,
                           start_invite_talk, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/start_talk_broadcast", XHttpServer,
                           start_talk_broadcast, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/scan_device_list", XHttpServer,
                           scan_device_list, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/query_device_library", XHttpServer,
                           query_device_library, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/refresh_device_library", XHttpServer,
                           refresh_device_library, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/start_playback", XHttpServer,
                           start_playback, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/fast_forward_playback", XHttpServer,
  ...
  HV_REGISTER_SYNC_HANDLER(router, POST, "/on_play", XHttpServer, on_play,
                           this);
  HV_REGISTER_SYNC_HANDLER(router, POST, "/on_stream_changed", XHttpServer,
                           on_stream_changed, this);
  HV_REGISTER_SYNC_HANDLER(router, GET, "/get_snap", XHttpServer, get_snap,
                           this);

  /* 异步请求 */
  HV_REGISTER_ASYNC_HANDLER(router, GET, "/refresh_device_library_async",
                            refresh_device_library_async, this);
  ...
  LOG(INFO) << fmt::format("http.worker_processes    :{}\n"
                           "http.worker_connectons   :{}\n"
                           "http.worker_threads      :{}\n",
                           server_.worker_processes, server_.worker_connections,
                           server_.worker_threads);
  return true;
}
...
std::string XHttpServer::query_device_library__(HttpRequest *req,
                                                HttpResponse *resp) {
  std::string device_id = req->GetParam("device_id");
  if (device_id.empty()) {
  ...
  for (const auto &record_info : record_infos) {
    if (!start_time.empty() &&
        record_info->start_time.compare(start_time) < 0) {
      break;
    }
...
int XHttpServer::refresh_record_history__(HttpRequest *req,
                                          HttpResponse *resp) {
  std::string device_id = req->GetParam("device_id");
  if (device_id.empty()) {
    return -1;
...

  {
    ...
    sscanf(start_time.c_str(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour,
           &min, &sec);
    sprintf(sz_start_time, "%d-%02d-%02dT%02d:%02d:%02d", year, month, day,
            hour, min, sec);
    sscanf(end_time.c_str(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour,
           &min, &sec);
    sprintf(sz_end_time, "%d-%02d-%02dT%02d:%02d:%02d", year, month, day, hour,
            min, sec);
    XmlQueryLibraryParamPtr params_ptr =
        std::make_shared<XmlQueryLibraryParam>();
    ...
    LOG(INFO) << fmt::format(
        "params_ptr->start_time:{} params_ptr->end_time:{}",
        params_ptr->start_time, params_ptr->end_time);
  }
...
int XHttpServer::refresh_device_library_async(const HttpContextPtr &context) {
  if (refresh_record_history__(context->request.get(),
                               context->response.get()) < 0) {
    return context->response->String("refresh record history error!");
  }
  hv::async([context, this]() {
    Server::instance()->WaitHistory();
    // std::this_thread::sleep_for(std::chrono::seconds(10));
    std::string history_video_list =
        query_device_library__(context->request.get(), context->response.get());
...
int XHttpServer::get_snap(HttpRequest *req, HttpResponse *resp) {
  ...
  std::string rtsp_url =
      fmt::format("rtsp://{}/rtp/{}", gMediaServerInfo.rtp_ip, ssrc);
  ...
  std::string filepath =
      fmt::format("./imgs/{}_{}.jpg",
                  Xzm::util::Timer::instance()->XGetCurrentTime(), ssrc);
...
int XHttpServer::get_rtp_info(HttpRequest *req, HttpResponse *resp) {
  ...
  if (code != 0 || !exist) {
    LOG(ERROR) << fmt::format(
        "--------------------check_stream failed!,code:{} exist:{}", code,
        (exist ? 1 : 0));
    // return false;
  }
  resp->json["code"] = 0; // 鉴权成功
...
int XHttpServer::on_publish(HttpRequest *req, HttpResponse *resp) {
  ...
  LOG(INFO) << fmt::format("app:{}\n"
                           ...
                           "media_server_id:{}\n"
                           "rtsp_url:{}\n",
                           app, id, ip, params, port, schema, stream, vhost,
                           media_server_id, rtsp_url);
  resp->json["code"] = 0; // 鉴权成功
  resp->json["msg"] = "success";
  return kHttpOK; // http调用成功
}
