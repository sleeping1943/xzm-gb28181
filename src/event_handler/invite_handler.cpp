...
bool InviteHandler::Process(eXosip_event_t *evtp, eXosip_t *sip_context_,
                            int code) {
  LOG(INFO) << "invite handler Process!!";
...
bool InviteHandler::send_meida_data(const SendRtpInfoPtr ptr) {
  // 以下推流到摄像头逻辑修改为推流后执行
  std::string publish_url = fmt::format(
      "http://{}/index/api/startSendRtp?secret={}&vhost={}&app={}"
      "&stream={}&src_port={}&ssrc={}&dst_url={}&dst_port={}&is_udp={}&pt={}&"
      "use_ps={}&only_audio={}",
      gMediaServerInfo.rtp_ip, ptr->secret, ptr->host, ptr->app, ptr->stream_id,
      ptr->talk_port, ptr->stream_id, ptr->client_ip, ptr->client_port,
      ptr->is_udp, ptr->pt, ptr->use_ps, ptr->only_audio);
  LOG(INFO) << fmt::format("url:%s", publish_url);
  auto resp = requests::get(publish_url.c_str());
  auto ret_json = resp->GetJson();
  int publish_code = 0;
  HV_JSON_GET_INT(ret_json, publish_code, "code");
  if (publish_code != 0) {
    LOG(ERROR) << fmt::format("startSendRtp failed!,code:{}", publish_code);
    return false;
  }
  return true;
}
...
std::string InviteHandler::parse_ssrc(const std::string &str_body) {
  ...
  if (!ssrc.empty()) {
    LOG(INFO) << fmt::format("ssrc :{}", ssrc);
  }
  return ssrc;
}

bool InviteHandler::replay_invite(eXosip_event_t *evtp, eXosip_t *sip_context_,
                                  osip_body_t *body,
                                  SendRtpInfoPtr send_info_ptr) {
  ...
  do {
    ...
    std::string str_client_info =
        fmt::format("\nclient_ip:{}\tusername:{}\tsession_id:{}\tsession_ver:{}"
                    "\tclient_port:{}\tstr_proto:{}\tmedia_type:{}\n",
                    send_info_ptr->client_ip, username, session_id, session_ver,
                    send_info_ptr->client_port, str_proto, media_type);
    LOG(INFO) << fmt::format("sip_client_info:{}", str_client_info);
    dump_request(evtp);
    dump_response(evtp);
    // 找到对应的client
    client_ptr = Server::instance()->FindClientEx(username);
    if (client_ptr == nullptr) {
      LOG(ERROR) << fmt::format("can not find the client:{}", username);
      break;
    }
    if (client_ptr->is_talking) {
      client_ptr = nullptr;
      LOG(ERROR) << "client is talking";
      break;
    }
    ...
    if (ret != 0) {
      LOG(ERROR) << "InviteHandler:eXosip_call_build_ack failed!";
      break;
    }
    send_info_ptr->talk_port = Server::instance()->GetTalkPort();
    if (send_info_ptr->talk_port <= 0) {
      LOG(ERROR) << "can not get port for mediakit to speek";
      break;
    }
    auto md5_str = Xzm::MD5(username).toStr();
    std::transform(md5_str.begin(), md5_str.end(), md5_str.begin(),
    ...
    char sdp[2048] = {0};
    {
      snprintf(sdp, 2048,
               ...
               "y=%s\r\n"
               "f=v/////a/1/8/1\r\n",
               s_info.sip_id.c_str(), media_info.rtp_ip.c_str(),
               media_info.rtp_ip.c_str(), media_type.c_str(),
               send_info_ptr->talk_port, str_proto.c_str(),
               send_info_ptr->ssrc.c_str());
    }
    osip_message_set_body(msg, sdp, strlen(sdp));
    osip_message_set_content_type(msg, "application/sdp");
    ret = eXosip_call_send_answer(sip_context_, evtp->tid, 200, msg);
    if (ret == 0) {
      LOG(INFO) << "InviteHandler:eXosip_call_send_answer OK";
    } else {
      LOG(ERROR) << fmt::format(
          "InviteHandler:eXosip_call_send_answer error={}", ret);
    }
  } while (0);
  if (sdp_msg) {
  ...
  if (client_ptr == nullptr) {
    LOG(ERROR) << fmt::format(
        "can not find the client[{}] or client is talking!", username);
    return false;
  }
  return true;
}
