...
bool CallAnswerHandler::Process(eXosip_event_t *evtp, eXosip_t *sip_context_,
  ...
  if (!ret && msg) {
    ...
    LOG(INFO) << fmt::format("eXosip_call_send_ack OK, Add PublishStreamInfo, "
                             "ssrc[{}], cid[{}], did[{}]",
                             ssrc.c_str(), evtp->cid, evtp->did);
  } else {
    LOG(ERROR) << fmt::format("eXosip_call_send_ack error={}", ret);
  }
  return true;
}
}; // namespace Xzm
