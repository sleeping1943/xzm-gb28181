...
using FUNC_MSG_RESPONSE =
    std::function<void(eXosip_event_t *evtp, eXosip_t *sip_context_, int code,
...
public:
  ...
  void response_message_answer(eXosip_event_t *evtp, eXosip_t *sip_context_,
                               int code);
  ...
  /* 刷新历史记录缓存 */
  int request_refresh_device_library(eXosip_t *sip_context,
   ...
   * @return int
   */
  int generate_borad_cast_xml(char *str_from, char *str_to, char *str_body,
                              ClientInfoPtr client_info_ptr, ClientPtr client);
  void response_catalog(eXosip_event_t *evtp, eXosip_t *sip_context_, int code,
                        std::shared_ptr<boost::any> param);
  void response_recordinfo(eXosip_event_t *evtp, eXosip_t *sip_context_,
...
private:
  ...
  std::map<DeviceID, std::atomic_int>
      history_video_cache_; // <device_id, 当前解析出的历史录像个数>
  std::thread thread_;
};
