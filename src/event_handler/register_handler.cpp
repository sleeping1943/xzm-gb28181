...
bool RegisterHandler::Process(eXosip_event_t *evtp, eXosip_t *sip_context_,
                              int code) {
  ...
  if (MSG_IS_REGISTER(evtp->request)) { // 注册客户端消息
    ...
    if (body) {
      ...
      parse_xml(body->body, "<DeviceID>", false, "</DeviceID>", false,
                DeviceID);
    }
...
bool RegisterHandler::register_client(eXosip_event_t *evtp,
  ...
  ClientPtr client = std::make_shared<Client>(
      contact->url->host, atoi(contact->url->port), contact->url->username);
  ...
  do {
    ...
    LOG(INFO) << fmt::format("user_agent[name:{} value:{}]", user_agent->hname,
                             user_agent->hvalue);
  ...
  do {
    if (!check_ha1(evtp, auth)) { // 鉴权没通过
      ...
      LOG(INFO) << fmt::format(
          "Camera registration error, p={}},port={},device={}",
          client->ip.c_str(), client->port, client->device.c_str());
      break;
    }
    ...
    //}
    LOG(INFO) << fmt::format(
        "IP Camera registration success,ip={},port={},device={},type:{}",
        client->ip.c_str(), client->port, client->device.c_str(),
...
void RegisterHandler::response_register_401unauthorized(
    eXosip_event_t *evtp, eXosip_t *sip_context_) {
  char *dest = nullptr;
...
bool RegisterHandler::check_ha1(eXosip_event_t *evtp,
  ...
  char *method = NULL,      // REGISTER
      *algorithm = NULL,    // MD5
          *username = NULL, // 340200000013200000024
              *realm =
                  NULL, // sip服务器传给客户端，客户端携带并提交上来的sip服务域
                  *nonce =
                      NULL, // sip服务器传给客户端，客户端携带并提交上来的nonce
                      *nonce_count = NULL,
  ...
  auto s_info = gServerInfo;
  DigestCalcHA1(algorithm, username, realm, s_info.passwd.c_str(), nonce,
                nonce_count, HA1);
  DigestCalcResponse(HA1, nonce, nonce_count, auth->cnonce, auth->message_qop,
  ...
  DigestCalcResponse(temp_HA1, s_info.nonce.c_str(), NULL, NULL, NULL, 0,
                     method, uri, NULL, temp_response);
