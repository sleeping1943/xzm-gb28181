...
bool Server::Init(const std::string &conf_path) {
  ...
  REGISTER_MSG_RESPONSE("RecordInfo", &Handler::response_recordinfo,
                        kDefaultHandler)
...
bool Server::UpdateClientInfo(
  ...
  for (const auto &iter : client_infos) {
    ...
    if (registed_clients.empty() ||
        registed_clients.count(client_info->device_id) <= 0) { // 没有该设备
...
void Server::AddRecordInfo(const std::string &parent_device_id,
...
Server::GetRecordInfo(const std::string &parent_device_id) {
...
void Server::AddLivingInfoPtr(const std::string &stream_id,
                              LivingInfoPtr info) {
...
bool Server::run() {
  while (true) {
    ...
    eXosip_event_t *evtp =
        eXosip_event_wait(sip_context_, 0, 20); // 接受时间20ms超时
